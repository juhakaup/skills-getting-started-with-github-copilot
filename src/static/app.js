document.addEventListener("DOMContentLoaded", () => {
  ...
  activitiesList.addEventListener("click", async (event) => {
    const removeBtn = event.target.closest(".remove-participant");
    if (!removeBtn) return;

    const activity = decodeURIComponent(removeBtn.dataset.activity);
    const email = decodeURIComponent(removeBtn.dataset.email);
